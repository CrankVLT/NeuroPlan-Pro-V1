...
import { OKRHistoryDetail } from './planning/OKRHistoryDetail';
import { GoalCard } from './planning/GoalCard';
import { PerformancePieChart } from './planning/PerformancePieChart';
                    ...
                    <div className="grid lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            ...
                            {historyGoals.map(({ g: goal, type }) => {
                                ...
                                const progressColor = progress >= data.settings.executionThreshold ? 'text-neuro-green' : progress >= (data.settings.executionThreshold / 2) ? 'text-yellow-400' : 'text-neuro-red';

                                return (
                                    <div
                                        key={goal.id}
                                        onClick={() => setViewingHistoryGoal(goal)}
                                        className="glass p-4 rounded-xl border border-slate-800 opacity-75 hover:opacity-100 transition-opacity relative group cursor-pointer"
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="flex flex-col items-end gap-1">
                                                ...
                                                <span className={`font-mono text-xl font-black ${progressColor}`}>
                                                    {progress}%
                                                </span>
                                            </div>
                                        </div>
                                        <div className="text-[10px] text-slate-500 mt-2 font-mono flex justify-between items-center">
                                            ...
                                            <button
                                                ...
                                                className="p-2 hover:text-neuro-red text-slate-600 transition-colors text-xl font-bold"
                        ...
                        <div className="lg:col-span-1 hidden lg:block sticky top-24 h-fit">
                            <PerformancePieChart
                                goals={historyGoals.map(i => i.g)}
                                threshold={data.settings.executionThreshold}
                            />
                        </div>
                    </div>
                )}
